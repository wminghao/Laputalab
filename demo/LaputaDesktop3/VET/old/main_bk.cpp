...
int main()
{
    ...
    #endif
        Mat curCVMat4 = genMVPMat3x4(glasses, P_next);
        ...
        while (++ curIter < maxIter) {
            ...
            for (int i = 0; i < NP; i++){
                ...
                for (int k = -K/2; k <= K/2; k++){
                    ...
                    Mat curCVMat = genMVPMat3x4(glasses, P_tmp);
                    genSFImgGL(image, faces, vertices_tmp, sfVertices, map, sfImgTmp, curCVMat);
            ...
            Mat curCVMat = genMVPMat3x4(glasses, P_next);
            genSFImgGL(image, faces, vertices_next, sfVertices, map, sfImgNext, curCVMat);
            ...
            if (abs(err_next - err_cur) < errDiff) {
                ...
                while (err_next > err_cur) {
                    ...
                    Mat curCVMat = genMVPMat3x4(glasses, P_next);
                    genSFImgGL(image, faces, vertices_tmp1, sfVertices, map, sfImgNext, curCVMat);
        ...
        if (err_cur <= errTh) {
            ...
            */
            ...
            if (GLASSON) {
                ...
                Mat curRotTransMat = P2PMat(P_next);
                glm::mat4 curRotTransMat4;
                mat3x4ToMat4(&curRotTransMat, curRotTransMat4);
                drawOpenGLGlasses(dstTexture, frame, glasses, window, curRotTransMat4 );
