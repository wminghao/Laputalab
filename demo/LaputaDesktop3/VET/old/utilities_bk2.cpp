...
void drawMeshOnImgGL(Mat image, vector<triangle> & faces, vector<myvec3> vertices, Mat PMat)
{
    for (int i = 0; i < faces.size(); i++){
        vector<Point> contour;
        myvec3 v_a = vertices[faces[i].a];
        ...
        Mat vertex3D_1 = (Mat_<float>(4,1) << v_a.x, v_a.y, v_a.z, 1);
        Mat pt2D_1 = PMat*vertex3D_1;
        Mat vertex3D_2 = (Mat_<float>(4,1) << v_b.x, v_b.y, v_b.z, 1);
        Mat pt2D_2 = PMat*vertex3D_2;
        Mat vertex3D_3 = (Mat_<float>(4,1) << v_c.x, v_c.y, v_c.z, 1);
        ...
        contour.push_back(Point(pt2D_1.at<float>(0), V_HEIGHT-pt2D_1.at<float>(1)));
        contour.push_back(Point(pt2D_2.at<float>(0), V_HEIGHT-pt2D_2.at<float>(1)));
        contour.push_back(Point(pt2D_3.at<float>(0), V_HEIGHT-pt2D_3.at<float>(1)));
    ...
    for (int j = 0; j < SF_HEIGHT; j++)
        for (int i = 0; i < SF_WIDTH; i++){
            if (map[j][i] != 255){
                ...
                float v1y = V_HEIGHT-pt2D_1.at<float>(1);
                float v2y = V_HEIGHT-pt2D_2.at<float>(1);
                float v3y = V_HEIGHT-pt2D_3.at<float>(1);
...
void mat4ToMat3x4(glm::mat4& pMat4, Mat* pMat3x4)
{
    ...
    for (int i = 0; i < 3; ++i) {
        for(int j = 0; j< 3; ++j ) {
            pMat3x4->at<float>(i, j) = pMat4[i][j];
            //printf("mat4ToMat3x4: pMat4[%d][%d] = %f\r\n", i, j, pMat4[i][j]);
        }
    }
    //reverse for translation
    for(int j = 0; j < 3; ++j ) {
        pMat3x4->at<float>(j, 3) = pMat4[3][j];
        //printf("mat4ToMat3x4: pMat4[%d][%d] = %f\r\n", 3, j, pMat4[3][j]);
    }
    for(int i = 0; i< 4; ++i ) {
        //printf("mat4ToMat3x4: pMat4[%d][%d] = %f\r\n", i, 3, pMat4[i][3]);
...
void mat3x4ToMat4(Mat* pMat3x4, glm::mat4& pMat4)
{
    ...
    for (int i = 0; i < 3; ++i) {
        for(int j = 0; j< 3; ++j ) {
            pMat4[i][j] = pMat3x4->at<float>(i, j);
            //printf("mat3x4ToMat4: pMat4[%d][%d] = %f\r\n", i, j, pMat4[i][j]);
        }
    }
    //reverse for translation
    for(int j = 0; j < 3; ++j ) {
        pMat4[3][j] = pMat3x4->at<float>(j, 3);
        //printf("mat3x4ToMat4: pMat4[%d][%d] = %f\r\n", 3, j, pMat4[3][j]);
    }
...
/*
Mat P2PMatNew(float P[]){
    Mat res = Mat::zeros(3, 4, CV_32F);
    
    Mat rX = (Mat_<float>(3,3) << 1, 0, 0, 0, cos(P[0]), -sin(P[0]), 0, sin(P[0]), cos(P[0]));
    Mat rY = (Mat_<float>(3,3) << cos(P[1]), 0, sin(P[1]), 0, 1, 0, -sin(P[1]), 0, cos(P[1]));
    Mat rZ = (Mat_<float>(3,3) << cos(P[2]), -sin(P[2]), 0, sin(P[2]), cos(P[2]), 0, 0, 0, 1);
    Mat Rot = rZ*rY*rX;
    
    res.at<float>(0,0) = Rot.at<float>(0,0);
    res.at<float>(0,1) = Rot.at<float>(0,0);
    res.at<float>(0,2) = Rot.at<float>(0,0);
    res.at<float>(0,3) = P[3];
    res.at<float>(1,0) = Rot.at<float>(0,0);
    res.at<float>(1,1) = Rot.at<float>(0,0);
    res.at<float>(1,2) = Rot.at<float>(0,0);
    res.at<float>(1,3) = P[4];
    res.at<float>(2,0) = Rot.at<float>(0,0);
    res.at<float>(2,1) = Rot.at<float>(0,0);
    res.at<float>(2,2) = Rot.at<float>(0,0);
...
Mat P2PMat(float P[]){
    ...
    res.at<float>(0,0) = cos(P[1])*cos(P[2]);
    res.at<float>(0,1) = cos(P[0])*sin(P[2]) + sin(P[0])*sin(P[1])*cos(P[2]);
    res.at<float>(0,2) = sin(P[0])*sin(P[2]) - cos(P[0])*sin(P[1])*cos(P[2]);
    res.at<float>(0,3) = P[3];
    res.at<float>(1,0) = -cos(P[1])*sin(P[2]);
    res.at<float>(1,1) = cos(P[0])*cos(P[2]) - sin(P[0])*sin(P[1])*sin(P[2]);
    res.at<float>(1,2) = sin(P[0])*cos(P[2]) + cos(P[0])*sin(P[1])*cos(P[2]);
    res.at<float>(1,3) = P[4];
    res.at<float>(2,0) = sin(P[1]);
    res.at<float>(2,1) = -sin(P[0])*cos(P[1]);
    res.at<float>(2,2) = cos(P[0])*cos(P[1]);
    res.at<float>(2,3) = P[5];
